...
/**
 * Renderer for outputting the extresources course format.
 *
 * @package format_extresources
...
require_once($CFG->dirroot.'/course/format/extresources/locallib.php');

/**
 * Basic renderer for extresources format.
 *
 * @copyright 2012 Dan Poltawski
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_extresources_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        // Since format_extresources_renderer::section_edit_controls() only displays the 'Set current section' control when editing mode is on
    ...
    protected function page_title() {
        ...
        return get_string('pluginname', 'format_extresources');
    }
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        // allow extresources types
        $config = get_config('format_extresources');
        ...
        foreach ($forumsection as $section => $thissection){
            ...
            if (!$PAGE->user_is_editing() && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
                ...
                //echo $this->section_summary($thissection, $course, null); // extresources doesn't show summary, jk
            } else {
                echo $this->section_header($thissection, $course, false, 0);
                if ($thissection->uservisible) {
                    //echo $this->courserenderer->course_section_cm_list($course, $thissection, 0); // show extresources
                    echo $this->local_section_cm_list($course, $thissection, $allowtypes, 0); // local show extresources(only extresources)
    ...
    private function local_section_add_cm_control($course, $section, $allowtypes, $sectionreturn = null, $displayoptions = array()) {
        ...
        // Retrieve all modules with associated metadata, jk: retrieve module with extresources
        /// $modules = get_module_metadata($course, $modnames, $sectionreturn); 
        $modules = format_extresources_get_module_metadata($course, $modnames, $sectionreturn); // change mod->link(from /course/mod.php => /blocks/xxx/mod.php)
        $urlparams = array('section' => $section);

        // We'll sort extresources and activities into two lists
