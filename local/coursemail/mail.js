...
YUI(M.yui.loader).use('io-base', 'node', 'json-parse', 'panel', 'datatable-base', 'dd-plugin', 'moodle-form-dateselector', function(Y) {
    ...
    var mail_create_edit_label_panel = function () {
        var title = M.util.get_string('editlabel', 'local_coursemail');
        ...
        mail_edit_label_panel = new Y.Panel({
            srcNode      : '#local_coursemail_form_edit_label',
            headerContent: title,
            width        : width,
            zIndex       : 5,
            centered     : false,
    ...
    var mail_create_new_label_panel = function () {
        var title = M.util.get_string('newlabel', 'local_coursemail');
        var obj = (Y.one('.mail_list')?Y.one('.mail_list'):Y.one('.mail_view'));
        var position = obj.getXY();
        var width = 400;
        var posx = position[0]+(Y.one('body').get('offsetWidth')/2)-width;
        mail_new_label_panel = new Y.Panel({
            srcNode      : '#local_coursemail_form_new_label',
            headerContent: title,
            width        : width,
            zIndex       : 5,
            centered     : false,
...
//                }
    ...
    var mail_enable_all_buttons = function(bool) {
        ...
        if (Y.one('#mail_compose')) {
            mail_enable_button(Y.one('#mail_compose'), true);
        }
    ...
    var handleSuccess = function (transactionid, response, args) {
        ...
        if (obj.msgerror) {
                ...
                if (obj.info.root) {
                    ...
                    if(node){
                        ...
                        if(obj.info.root.match(/\(\d+\)/)) {
                            Y.one('.mail_root').addClass('local_coursemail_new_messages');
                        } else {
                            Y.one('.mail_root').removeClass('local_coursemail_new_messages');
                        }
                    }
                }
                if (obj.info.inbox) {
            ...
            if (obj.undo && mail_undo_function != 'undo') {
                var msg = M.util.get_string('undo'+mail_undo_function, 'local_coursemail', obj.undo.split(',').length);
    ...
    var mail_doaction = function(action, node) {
        ...
        if(mail_message_view) {
            if(action == 'togglestarred') {
                ...
                if (obj.hasClass('mail_starred')) {
                    ...
                    node.set('title', M.util.get_string('unstarred','local_coursemail'));
                } else {
                    action = 'starred';
                    obj.replaceClass('mail_unstarred', 'mail_starred');
                    node.set('title', M.util.get_string('starred','local_coursemail'));
                }
                ...
                node.ancestor('a').set('title', M.util.get_string('starred','local_coursemail'));
            } else if (action == 'unstarred') {
                node = Y.one('.mail_flags span');
                node.replaceClass('mail_starred', 'mail_unstarred');
                node.ancestor('a').set('title', M.util.get_string('unstarred','local_coursemail'));
                ...
                nodes.each(function (node) {
                    ...
                    if (action == 'starred') {
                        ...
                        if(child) {
                            ...
                            child.ancestor('a').set('title', M.util.get_string('starred','local_coursemail'));
                        }
                    } else if(action == 'unstarred') {
                        if (mail_view_type == 'starred') {
                            ...
                            if(child) {
                                ...
                                child.ancestor('a').set('title', M.util.get_string('unstarred','local_coursemail'));
                            }
                        }
        ...
        if (action == 'setlabel') {
            ...
            if (!cfg.data.labelname) {
                alert(M.util.get_string('erroremptylabelname', 'local_coursemail'));
                mail_label_edit();
                return false;
            }
        ...
        if (action == 'newlabel') {
            obj = Y.one('#local_coursemail_new_label_color');
            cfg.data.labelname = Y.one('#local_coursemail_new_label_name').get('value');
            if (!cfg.data.labelname) {
        ...
        request = Y.io(M.cfg.wwwroot + '/local/coursemail/ajax.php', cfg);
    };
    ...
    var mail_label_confirm_delete = function(e) {
        ...
        message = M.util.get_string('labeldeleteconfirm', 'local_coursemail', M.local_mail.mail_labels[labelid].name);
        M.util.show_confirm_dialog(e, {
                                        'callback' : mail_label_remove,
                                        'message' : message,
                                        'continuelabel': M.util.get_string('delete', 'local_coursemail')
                                    }
        );
    };
    ...
    var mail_label_new = function() {
        ...
        Y.one('#local_coursemail_form_new_label').removeClass('mail_hidden');
        Y.one('#local_coursemail_new_label_name').focus();
    };
    ...
    var mail_label_edit = function() {
        ...
        Y.one('#local_coursemail_form_edit_label').removeClass('mail_hidden');
    ...
    var mail_update_url = function() {
        ...
        if (history.pushState) {
            ...
            history.pushState({}, document.title, M.cfg.wwwroot + '/local/coursemail/view.php?' + params.join('&'));
        }
    };
