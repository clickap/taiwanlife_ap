...
class mod_forum_post_form extends moodleform {
    ...
    function definition() {
        ...
        /*elaine:add option
        add 是否上架及上架期間、公佈至課程、公佈至首頁 
        */
        $fchoices = array();
        $fchoices['0'] = get_string('hide');
        $fchoices['1'] = get_string('show');
        if($forum->type=='news'){   
            $mform->addElement('select', 'visible', get_string('visible'), $fchoices);
            $mform->setDefault('visible', 1);
            if($course->id == SITEID){
                $mform->addElement('select', 'home_visible', get_string('home_publish'), $fchoices);
                $mform->setDefault('home_visible', 1);
                
                $mform->addElement('select', 'course_visible', get_string('course_publish'), $fchoices);
                $mform->setDefault('course_visible', 1); 
            }
            //$mform->addElement('date_selector', 'startdate', get_string('startpublish'));
            //$mform->setDefault('startdate', time() + 3600 * 24);  
            //$mform->addElement('date_selector', 'enddate', get_string('endpublish'));
