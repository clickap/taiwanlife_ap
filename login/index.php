...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if(!empty($CFG->recaptchapublickey) && !empty($CFG->recaptchaprivatekey) && get_config('auth/email', 'recaptcha')){
            if (empty($errormsg)) {
                if($frm->recaptcha_response_field){ 
                    require_once $CFG->libdir . '/recaptchalib.php';
                    $recaptcha_element = $frm->recaptcha_element;
                    if (!empty($frm->recaptcha_challenge_field)) {
                        $challenge_field = $frm->recaptcha_challenge_field;
                        $response_field = $frm->recaptcha_response_field;     //input value
                        $remoteip = getremoteaddr(null);     
                        $response = recaptcha_check_answer($CFG->recaptchaprivatekey, $remoteip, $challenge_field, $response_field);
                    ...
                    $errormsg = get_string('recaptcha').'Error.';
                }
            }
        }
        /**/
    ...
    /// Prepare redirection
        if (user_not_fully_set_up($USER)) {
            ...
            //mary : org: $urltogo = $CFG->wwwroot.'/';
            //after login redirct to my courses page
            $urltogo = $CFG->wwwroot.'/admin/tool/mooccourse/mycourses.php';
            unset($SESSION->wantsurl);
        }
