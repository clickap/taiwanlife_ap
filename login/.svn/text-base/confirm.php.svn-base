...
if (!empty($data) || (!empty($p) && !empty($s))) {
    ...
    if ($confirmed == AUTH_CONFIRM_ALREADY) {
        ...
        //mary:org:echo $OUTPUT->single_button("$CFG->wwwroot/course/", get_string('courses'));
        echo $OUTPUT->single_button("$CFG->wwwroot/all_course_list.php", get_string('availablecourses'));
        echo $OUTPUT->box_end();
        echo $OUTPUT->footer();
        exit;
