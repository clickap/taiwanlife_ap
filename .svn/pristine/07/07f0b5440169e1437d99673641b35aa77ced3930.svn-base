        ...
        switch ($v) {     /// Display the main part of the front page.
            case FRONTPAGENEWS:
                if ($SITE->newsitems) { // Print forums only when needed
                    ...
                    ewantforum_print_latest_discussions($SITE, $newsforum, $SITE->newsitems, 'plain', 'p.modified DESC');
            ...
            case FRONTPAGECATEGORYNAMES:
                ...
                // eaine : CaourseCategoty 課程類別顯示方式*/
                //org:echo $courserenderer->frontpage_categories_list();
                //require_once($CFG->dirroot .'/course/renderer.php');
                $mooc_courserenderer = $PAGE->get_renderer('tool_mooccourse');
                echo $mooc_courserenderer->frontpage_categories_list_img();
            ...
            break;
            case FRONTPAGENEWCOURSELIST:/*elaine:add 最新開課 */
                $mooc_courserenderer = $PAGE->get_renderer('tool_mooccourse');
                $availablecourseshtml = $mooc_courserenderer->frontpage_available_mooccourses(true);
                if (!empty($availablecourseshtml)) {
                    echo html_writer::tag('a', get_string('skipa', 'access', textlib::strtolower(get_string('latestcourses', 'tool_mooccourse'))), array('href'=>'#skipavailablecourses', 'class'=>'skip-block'));  //row-fluid" id="middle-blocks"
                    echo html_writer::start_tag('div', array('class'=>'row-fluid', 'id'=>'middle-blocks'));
                    echo $OUTPUT->heading(get_string('latestcourses', 'tool_mooccourse'), 2, 'headingblock header');
                    echo $availablecourseshtml;
                    echo html_writer::end_tag('div');

                }
                // mnet
                $mnetavailablecourseshtml = $mooc_courserenderer->frontpage_available_mnetcourses(true);
                if (!empty($mnetavailablecourseshtml)) {
                    echo html_writer::tag('a', get_string('skipa', 'access', textlib::strtolower(get_string('latestmnetcourses', 'tool_mooccourse'))), array('href'=>'#skipavailablecourses', 'class'=>'skip-block'));  //row-fluid" id="middle-blocks"
                    echo html_writer::start_tag('div', array('class'=>'row-fluid', 'id'=>'middle-blocks'));
                    echo $OUTPUT->heading(get_string('latestmnetcourses', 'tool_mooccourse'), 2, 'headingblock header');
                    echo $mnetavailablecourseshtml;
                    echo html_writer::end_tag('div');
