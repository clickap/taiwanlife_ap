...
YUI(M.yui.loader).use('io-base', 'node', 'json-parse', 'panel', 'datatable-base', 'dd-plugin', function(Y) {
    ...
    var mail_create_recipients_panel = function () {
        var title = M.util.get_string('addrecipients', 'local_coursemail');
        var width = 500;
        var obj = Y.one('#region-main');
        var position = obj.getXY();
        var posx = position[0]+(Y.one('body').get('offsetWidth')/2)-width;

        if (Y.one('#local_coursemail_recipients_form')) {
            mail_recipients_panel = new Y.Panel({
                srcNode      : '#local_coursemail_recipients_form',
                headerContent: title,
                width        : width,
                autoScroll   : true,
                zIndex       : 3000,
            ...
            mail_recipients_panel.addButton({
                value  : M.util.get_string('applychanges', 'local_coursemail'),
                section: Y.WidgetStdMod.FOOTER,
                action : function (e) {
                    e.preventDefault();
                    mail_recipients_panel.hide();
//alert(mail_changes_recipients());
    ...
    var handleSuccess = function (transactionid, response, args) {
        ...
        if (obj.msgerror) {
            ...
            if (obj.html) {
                ...
                if (node) {
                    node.setContent(M.util.get_string('emptyrecipients','local_coursemail'));
                }
            }
            if (obj.info) {
        ...
        if (action == 'updaterecipients') {
            if (mail_existing_recipients) {
                Y.Array.each(mail_existing_recipients, function(recipient, index) {
                    if ((idx=Y.Array.indexOf(mail_recipients[3], recipient)) != -1) {
    ...
    var getgroupid = function() {
        var select = Y.one('#local_coursemail_recipients_groups');
    ...
    var mail_recipient_action = function(node) {
        ...
        var userid = /\[(\d+)\]/.exec(name)[1]; // not used
        var noderolestring;
        ...
        if (node.get('type') === 'image') {
            ...
            noderolestring = node.ancestor('.mail_recipient_actions').siblings('.mail_form_recipient_role');
            noderolestring.set('innerHTML', M.util.get_string('shortadd'+role, 'local_coursemail')+':');
...
//alert('mail_recipients:['+ mail_recipients + '], role: "' + role +'", userid: "' + userid +'"');
...
//alert(mail_recipients[0].length + '-' + mail_recipients[1].length + '-' + mail_recipients[2].length + '-' + mail_recipients[3].length);
    ...
    var mail_update_recipients_state = function () {
        ...
        Y.Array.each(mail_recipients[0], function (recipient) {
            ...
            if (node) {
                node.set('innerHTML', M.util.get_string('shortaddto', 'local_coursemail')+':');
        ...
        Y.Array.each(mail_recipients[1], function (recipient) {
            ...
            node = Y.one('span.mail_form_recipient_role[data-role-recipient="'+recipient+'"]');
            if (node) {
                node.set('innerHTML', M.util.get_string('shortaddcc', 'local_coursemail')+':');
                node.removeClass('mail_hidden');
                node.ancestor('.mail_form_recipient').addClass('mail_recipient_selected');
            }
            node = Y.one('.mail_recipient_actions input[name="remove['+recipient+']"]');
        ...
        Y.Array.each(mail_recipients[2], function (recipient) {
            ...
            node = Y.one('span.mail_form_recipient_role[data-role-recipient="'+recipient+'"]');
            if (node) {
                node.set('innerHTML', M.util.get_string('shortaddbcc', 'local_coursemail')+':');
                node.removeClass('mail_hidden');
                node.ancestor('.mail_form_recipient').addClass('mail_recipient_selected');
            }
            node = Y.one('.mail_recipient_actions input[name="remove['+recipient+']"]');
    ...
    Y.one("#region-main").delegate('click', function(e) {
        ...
        if (mail_recipients_panel) {
            ...
            Y.one('#local_coursemail_recipients_form').removeClass('mail_hidden');
        } else {
            alert(M.util.get_string('notingroup', 'local_coursemail'));
        }
    }, '#id_recipients');
    ...
    }, '#local_coursemail_recipients_groups');
