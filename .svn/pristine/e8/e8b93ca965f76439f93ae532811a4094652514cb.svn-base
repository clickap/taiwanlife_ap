...
/**
 * Renderer for outputting the quizs course format.
 *
 * @package format_quizs
...
require_once($CFG->dirroot.'/course/format/quizs/locallib.php');

/**
 * Basic renderer for quizs format.
 *
 * @copyright 2012 Dan Poltawski
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_quizs_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        // Since format_quizs_renderer::section_edit_controls() only displays the 'Set current section' control when editing mode is on
    ...
    protected function page_title() {
        ...
        return get_string('pluginname', 'format_quizs');
    }
...
//DebugBreak();
        ...
        // allow quiz types
        $config = get_config('format_quizs');
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            if (!$PAGE->user_is_editing() && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
                ...
                //echo $this->section_summary($thissection, $course, null); // quiz doesn't show summary, jk
            } else {
                echo $this->section_header($thissection, $course, false, 0);
                if ($thissection->uservisible) {
                    //echo $this->courserenderer->course_section_cm_list($course, $thissection, 0); // show quiz
                    echo $this->local_section_cm_list($course, $thissection, $allowtypes, 0); // local show quiz(only quiz)
    ...
    private function local_section_add_cm_control($course, $section, $allowtypes, $sectionreturn = null, $displayoptions = array()) {
        ...
        // Retrieve all modules with associated metadata, jk: retrieve module with quiz
        /// $modules = get_module_metadata($course, $modnames, $sectionreturn); 
        $modules = format_quizs_get_module_metadata($course, $modnames, $sectionreturn); // change mod->link(from /course/mod.php => /blocks/xxx/mod.php)
        $urlparams = array('section' => $section);

        // We'll sort quizs and activities into two lists
