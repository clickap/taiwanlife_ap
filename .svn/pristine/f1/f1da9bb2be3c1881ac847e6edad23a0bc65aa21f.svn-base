...
/**
 * Renderer for outputting the resources course format.
 *
...
require_once($CFG->dirroot.'/course/format/resources/locallib.php');

/**
 * Basic renderer for resources format.
 *
 * @copyright 2012 Dan Poltawski
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_resources_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        // Since format_resources_renderer::section_edit_controls() only displays the 'Set current section' control when editing mode is on
    ...
    protected function page_title() {
        ...
        return get_string('pluginname', 'format_resources');
    }
...
//DebugBreak();
        ...
        $config = get_config('format_resources');
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            if (!$PAGE->user_is_editing() && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
                ...
                //echo $this->section_summary($thissection, $course, null); // resource doesn't show summary, jk
            } else {
                echo $this->section_header($thissection, $course, false, 0);
                if ($thissection->uservisible) {
                    //echo $this->courserenderer->course_section_cm_list($course, $thissection, 0); // show resource
                    echo $this->local_section_cm_list($course, $thissection, $allowtypes, 0); // local show resource(only resource)
    ...
    private function local_section_add_cm_control($course, $section, $allowtypes, $sectionreturn = null, $displayoptions = array()) {
        ...
        // Retrieve all modules with associated metadata, jk: retrieve module with resource
        /// $modules = get_module_metadata($course, $modnames, $sectionreturn); 
        $modules = format_resources_get_module_metadata($course, $modnames, $sectionreturn); // change mod->link(from /course/mod.php => /blocks/xxx/mod.php)
        ...
        foreach ($modules as $module) {
                ...
                if ($module->archetype == MOD_ARCHETYPE_RESOURCE
                    && in_array($module->name, $allowtypes)
                ) {
